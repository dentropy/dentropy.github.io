...
The properties of the `GeneratedAnalytics` class are maybe best to think of as "data reports." `GeneratedAnalytics` "reports" are refreshed by corresponding **[Methods](/undefined)**.
...
A `dict` array with the total number of characters entered via messages to chat by element in [`user_list`](/undefined).
...
A `dict` array with the total number of characters per element in [`topic_list`](/undefined).
...
A `dict` array with the total number of messages to chat by element in [`user_list`](/undefined).
...
A `dict` array with the total number of messages per element in [`topic_list`](/undefined).
...
A `dict` array with the number of users per element  in [`topic_list`](/undefined).
